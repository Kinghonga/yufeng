...
$.ajax({
    ...
    success:function(d){
        ...
        $(daily).each(function(index, el) {
            ...
            $("#list").append(`
                <li>
                    ...
                    <div class="weather">
                        <img src="./images/${this.code_day}.svg" alt="">
                        <p>${this.text_day}/${this.text_night}</p>
                    </div>
                    <div class="tempRange">
                        ${this.low}/${this.high}℃
                    </div>
        ...
        $("#pic").find("img").attr("src","./images/"+code+".svg").end().find('p').first().text(temperature+"℃").next().text(text);
        $("#info").html(`
